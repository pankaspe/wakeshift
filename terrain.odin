...
TERRAIN_RIM_THICKNESS :: PLAYER_RIM_THICKNESS
...
compute_terrain_points :: proc(world: World, is_floor: bool) -> [dynamic]rl.Vector2 {
	...
	x := -offset
	index := start_index
	for x < SCREEN_WIDTH + TERRAIN_SEGMENT_WIDTH {
		variation := TERRAIN_PROFILE[index % len(TERRAIN_PROFILE)]

		y: f32
		if is_floor {
			y = SCREEN_HEIGHT - TERRAIN_BASE_HEIGHT - variation
		} else {
...
draw_terrain_side :: proc(world: World, is_floor: bool) {
	...
	edge_y: f32 = is_floor ? SCREEN_HEIGHT : 0
	for p in points {
		append(&strip, p)
