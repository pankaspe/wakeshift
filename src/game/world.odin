...
*
* F4 WAS HERE, AND THE PLAYTEST TOOK IT OUT
*
* For one commit the world ran at 1.35 seconds per real second while the
* character was on the ceiling and 1.0 on the floor: one key chose where
* to be, and also how fast the game happened. It was built, measured,
* played and removed the same day, and both halves of that are worth
* keeping.
*
* **Why it went.** It made the player want to live on the ceiling to go
* faster, and that want is *false*. A run ends when the front reaches the
* character, the front gains per pixel scrolled, and depth is distance —
* so a run dies at the same distance and scores the same number at any
* pace. Going fast arrives at the same place sooner; it never gets
* further. The mechanic taught a lesson the economy does not pay, which
* is the worst kind there is, and it was doing it with the one key the
* whole game is made of. Difficulty went back to where C3 left it: the
* bricks.
*
* **Why the implementation must survive the idea.** The obvious way to
* build any of this is to raise scroll_speed, and it is wrong. An
* obstacle's screen x is
*
*     anchor + (arrival_time - elapsed_time) * scroll_speed
*
* so a higher speed multiplies the *distance* to everything not yet
* arrived. Measured on a real flip, that slid one obstacle 247 px and the
* right edge of the screen 322 px, and the world breathed horizontally on
* every press. It is not even faster in the sense anyone means: arrival
* times do not move, so only the spacing between them grows.
*
* What worked was leaving scroll_speed alone and advancing elapsed_time
* faster instead — a **clock, not a speed**. Nothing slides, because the
* mapping is anchored on a constant: the largest single-step move of any
* obstacle was 6.08 px, exactly one step's worth. Score, difficulty and
* the Corruption all followed for free, because every one of them is
* measured in scrolled distance. And the pool needed no revalidation,
* because every fairness window is (body + width) / scroll_speed *world*
* seconds and scroll_speed never moved.
*
* **Slancio (roadmap R6.3) will meet this exact wall**, since buying
* speed is the same arithmetic. Build it as a clock. The code is one
* commit back if it is ever wanted whole.
*/
package game
