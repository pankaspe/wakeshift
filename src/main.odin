...
*   1. draw_gameplay, the one helper that lives here rather than in a
*      package: shared by three DRAW cases below, but pure orchestration
*      of core/game/render calls, not itself a game or render concern
...
*   4. Main loop, split into two switches over game_state:
...
main :: proc() {
	...
	generator := game.new_pattern_generator(game.all_patterns, 2.0, .Dream)
	...
	for !rl.WindowShouldClose() && !should_quit {
		...
		switch game_state {
		case .MainMenu:
			if ui.update_menu(&main_menu) {
				if main_menu.selected == 0 {
					game.reset_run(&player, &world, &score, &obstacles, &generator, &lucidity)
					game_state = .Playing
				} else {
					should_quit = true
				}
		...
		case .Playing:
			...
			game.update_world(&world, rl.GetFrameTime(), game.tiers[tier_index].scroll_speed)

			// update the player (input, flip/transition state)
			game.update_player(&player, world)

			// update the score, scaled by the current Lucidity multiplier
			game.update_score(&score, player, rl.GetFrameTime(), game.get_score_multiplier(lucidity))
			...
			for obstacle in obstacles {
				if game.check_player_obstacle_collision(player, obstacle, world) {
					...
					if score.value > high_score {
						...
						platform.save_high_score(high_score)
					}
				}
			}
